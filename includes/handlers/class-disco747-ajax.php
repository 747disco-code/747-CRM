...
class Disco747_Ajax {
    ...
    public function handle_send_email_template() {
        try {
            ...
            $pdf_path = null;
            if ($attach_pdf) {
                $this->log('[Email] PDF richiesto - genero PDF aggiornato...');
                
                // ✅ FIX: Genera sempre un nuovo PDF per garantire che sia aggiornato con i dati correnti del database
                // Questo evita problemi di mismatch tra nome file nel DB e file reale su disco
                ...
                if ($pdf_generator) {
                    ...
                    if ($pdf_path && file_exists($pdf_path)) {
                        ...
                        $this->log('[Email] Dimensione PDF: ' . filesize($pdf_path) . ' bytes');
